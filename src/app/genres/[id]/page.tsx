...
export default async function GenreDetailPage({ params, searchParams }: PageProps) {
  ...
  const relatedTracks = buildGenreRelatedTracks(snapshots, range, genreId);

  return (
    <main className="mx-auto max-w-6xl px-4 py-8 pt-20 md:px-8 lg:pl-72 lg:pt-8">
      <Nav />
      <h1 className="mb-4 text-3xl font-bold">Genre Stats</h1>
        ...
        <>
          <article className="mt-6 rounded-2xl border border-[var(--stroke)] bg-[var(--panel)] p-5">
            <div className="flex items-center gap-4">
              <div className="flex h-20 w-20 items-center justify-center rounded-lg bg-[var(--panel-soft)] text-3xl font-bold text-[var(--accent)]">#</div>
              <div>
                <p className="text-2xl font-semibold">{genre.name}</p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
              <Stat label="Estimated listened" value={formatEstimatedDuration(genre.totalHours, displayUnit)} />
              <Stat label="Appearances" value={String(genre.appearances)} />
              <Stat label="Average score" value={genre.avgScore.toFixed(2)} />
            </div>
            <div className="mt-4">
          ...
          <section className="mt-6 rounded-2xl border border-[var(--stroke)] bg-[var(--panel)] p-5">
            <h2 className="text-lg font-semibold">Likely related tracks</h2>
            <p className="mt-1 text-sm text-[var(--muted)]">Estimated tracks weighted by this genre share across snapshots.</p>
